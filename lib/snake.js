...
Snake.prototype.blockToTheLeft  = function () {
    // debugger;
    if (this.board.findBlock(this.head.x - 10, this.head.y)) {
        return true
    } else {
        return false
    }
    //return this.board.findBlock(this.x - 1, this.y);
    //if there is a block to the left, the function should return true,
    //otherwise, the function should return false.
    //We want to refer to the board to utilize our find block function, but we will need to
    //build out our eat function so that we can use it in our tests and it will automatically
    //shovel new snake blocks into the board.blocks array.
}
